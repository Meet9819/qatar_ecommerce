...
class prescriptionorder_details {
  ...
  prescriptionorder_details({this.prescription_id,
    this.item_unit,
    this.item_code,
    this.itemname_en,
    this.img,
...
class Prescription_order_detailsDemo extends StatefulWidget {
  final id;
  Prescription_order_detailsDemo({Key key, @required this.id}) : super(key: key);
  //List data;
  @override
  _Prescription_order_detailsDemoState createState() => _Prescription_order_detailsDemoState();
}

class _Prescription_order_detailsDemoState extends State<Prescription_order_detailsDemo> {
  ...
  Future<List<prescriptionorder_details>> _fetchJobs() async {
    final jobsListAPIUrl = 'http://sharegiants.in/ruchi/prescription_order_details.php';
    var data = {'prescriptionid':widget.id};
    final response = await http.post(jobsListAPIUrl, body: json.encode(data));

    if (response.statusCode == 200) {
      List jsonResponse = json.decode(response.body);
      return jsonResponse.map((job) => new prescriptionorder_details.fromJson(job)).toList();
    } else {
