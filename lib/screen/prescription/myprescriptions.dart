...
//List data;
  factory Prescriptions.fromJson(Map<String, dynamic> json) {
    return Prescriptions(
      ...
      insurance_company_approval_note_file_path: json['insurance_company_approval_note_file_path'],
...
class _myprescriptionState extends State<myprescription> {
  ...
  Future<List<Prescriptions>> _fetchPrescriptions() async {
    ...
    var url = 'http://sharegiants.in/ruchi/myprescriptions.php';
    var response = await http.post(url, body: json.encode(data));

    List jsonResponse = json.decode(response.body);
    // _finalprice_= jsonResponse["price"].map((item) => new Item.fromJson(item)).toList();

    return jsonResponse.map((item) => new Prescriptions.fromJson(item)).toList();
  }
                            ...
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                Row( mainAxisAlignment: MainAxisAlignment.spaceBetween,children: <Widget>[
                                  ...
                                  if(data[index].statustitle=="REJECTED")(
                                      Padding(
                                          ...
                                          child:Chip(
                                            ...
                                            label: Text(data[index].statustitle,      style: TextStyle(
                                                fontSize: 14, fontWeight: FontWeight.bold,color: Colors.white),),
                                          )
                                      ))
                                  else  if(data[index].statustitle=="PARTIAL_APPROVAL")(
                                      Padding(
                                          padding: const EdgeInsets.only(right: 10),
                                          child:Chip(
                                  ...
                                  else  if(data[index].statustitle=="AWAITING_APPROVAL")(
                                        Padding(
                                            padding: const EdgeInsets.only(right: 10),
                                            child:Chip(
                                    ...
                                    else  if(data[index].statustitle=="APPROVED")(
                                          Padding(
                                              padding: const EdgeInsets.only(right: 10),
                                              child:Chip(
                                      ...
                                      else  if(data[index].statustitle=="PROCESSED")(
                                            Padding(
                                                padding: const EdgeInsets.only(right: 10),
                                                child:Chip(
                                                  backgroundColor: Colors.green,
